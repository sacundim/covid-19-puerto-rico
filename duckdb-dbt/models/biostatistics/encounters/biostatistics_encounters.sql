...
SELECT
    ...
    -- days before.
	COALESCE(bool_or(prev.collected_date >= cur.collected_date - INTERVAL 90 DAY
			            AND prev.positive),
             FALSE)
		AS followup_strict,
    ...
    COALESCE(bool_or(prev.positive AND (
                CASE cur.test_type
                    WHEN 'Molecular'
                    THEN prev.collected_date >= cur.collected_date - INTERVAL 90 DAY
                    WHEN 'Antígeno'
                    THEN prev.collected_date >= cur.collected_date - INTERVAL 21 DAY
                END)), FALSE)
		AS followup
