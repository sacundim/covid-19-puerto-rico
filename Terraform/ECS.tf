...
#################################################################################
##
## CDC COVID Tracker scraper task
##

resource "aws_ecs_task_definition" "cdc_vax_scrape_and_sync" {
  family = "cdc-vax-scrape-and-sync"
  tags = {
    Project = var.project_name
  }
  requires_compatibilities = ["FARGATE"]
  ...
  container_definitions = jsonencode([
    {
      name = "cdc-vax-downloader",
      image = "${data.aws_ecr_image.downloader.registry_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/${data.aws_ecr_image.downloader.repository_name}:${data.aws_ecr_image.downloader.image_tag}"
      cpu = 1024,
      memoryReservation = 2048,
      essential = true,
      command = ["scrape-cdc-vaccinations.sh"],
      environment = [
        {
          name = "S3_DATA_URL",
          value = "s3://${var.datalake_bucket_name}"
...
resource "aws_cloudwatch_event_rule" "cdc_vax_daily_download" {
  name        = "cdc-vax-daily-download"
  description = "Run the daily CDC vaccinations download."
  schedule_expression = "cron(55 3,15 * * ? *)"
}

resource "aws_cloudwatch_event_target" "cdc_vax_daily_download" {
  target_id = "cdc-vax-daily-download"
  rule = aws_cloudwatch_event_rule.cdc_vax_daily_download.name
  arn = aws_ecs_cluster.main.arn
  role_arn = aws_iam_role.ecs_events_role.arn
  ...
  ecs_target {
    ...
    platform_version    = "LATEST"
    task_count          = 1
    task_definition_arn = aws_ecs_task_definition.cdc_vax_scrape_and_sync.arn
